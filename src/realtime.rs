...
impl RealtimeStore {
    ...
    pub async fn refresh(
        ...
        for feed in &config.feeds {
            ...
            if feed.vehicle_positions_url.is_some() {
                match self.refresh_vehicle_positions(feed).await {
                    Ok(outcome) => {
    ...
    async fn refresh_trip_updates(
        ...
        for entity in feed_message.entity {
            ...
            if trip_descriptor.schedule_relationship == Some(3) {
                self.canceled_trips.insert(trip_index, ());
                continue;
            }

            let trip = &static_data.trips[trip_index];
            let mut stop_update_positions =
                Vec::<(usize, gtfs_rt::trip_update::StopTimeUpdate)>::new();
            ...
            for stop_pos in 0..trip.stop_times.len() {
                if let Some((position, update)) = stop_update_positions.get(next_update) {
                    if *position == stop_pos {
                        ...
                        let skipped = update.schedule_relationship == Some(1);
    ...
    async fn refresh_vehicle_positions(&self, feed: &FeedConfig) -> Result<VehicleRefreshOutcome> {
        let url = feed
